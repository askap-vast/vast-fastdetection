...
nodes = ['cpu{}'.format(i) for i in range(7, 21) if i not in [8, 10]]
...
# partition rule 
# cpu[1-12] within partition hw-32C768G
# cpu[13-20] within partition insp-28C1T
partilist = []
for node in nodelist:
    if node in ['cpu{}'.format(i) for i in range(1, 13)]:
        partilist.append("hw-32C768G")
    else:
        partilist.append("insp-28C1T")

print('partition', partilist)
...
#            fw.write('#SBATCH --nodelist={}'.format(nodelist[idx]) + '\n')
        ...
        fw.write('#SBATCH --comment=vast2023' + '\n')
        fw.write('\n')
...
#        fw.write('#SBATCH --partition=all-x86-cpu' + '\n')
        fw.write('#SBATCH --partition={}'.format(partilist[idx]) + '\n')
