...
path_images = os.path.join(path, 'images_'+stokes)
...
for i, beam in enumerate(beamlist):
    ...
    outdir = path_cand
    ## output file name 
    name = "SB" + sbid + '_' + beam + '_{:.2f}d_'.format(seplist[i]) + stokes

    ## ====================================
    ## get the imagelist with correct order
    imagelist = []
    for size in ["?", "??", "???", "????"]:
        tmp = glob.glob(os.path.join(folder, f'*{beam}.{stokes}_{size}.fits'))
